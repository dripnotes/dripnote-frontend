...
- **Commands**: 패키지 구동 혹은 스크립트 실행 시 `pnpm --filter @coffee-service/web <명령어>` 구조로 실행합니다.
...
- **Harness Engineering (Rule 22)**: 실제 백엔드 API가 준비되지 않은 개발 단계에서는, 독립적인 UI 개발과 검증을 위해 반드시 모의 데이터(`mock`)나 전용 콜백 경로 등 **하네스(Harness) 환경**을 구축하여 개발 생산성을 유지해야 합니다.
