...
> **변경 사유 (Context)**: 커피는 향·맛·질감 등 감각적 경험이 핵심인 제품입니다. 텍스트 설명보다 대표 아로마의 식재료 이미지와 그에서 추출한 배경 색상을 통해 원두의 개성을 직관적으로 전달하는 방식이 Dripnote 브랜드 무드("Internal Coffee Lab")와 가장 잘 부합합니다.
...
│  GlobalNav                              │
│─────────────────────────────────────────│
│  [ 검색창 (BeanSearchBar) ]             │  ← 헤더 영역
│─────────────────────────────────────────│
│  ┌──────────┐  ┌───────────────────────┐│
│  │ Filter   │  │  BeanCardList         ││  ← 본문 영역
│  │ Panel    │  │  (원두 카드 그리드)    ││
│  │          │  │                       ││
│  └──────────┘  └───────────────────────┘│
└─────────────────────────────────────────┘
...
- **부모 컴포넌트**: `beans/layout.tsx` 또는 `beans/page.tsx`
...
interface BeanSearchBarProps {
  ...
  placeholder?: string; // 기본값: "원두 이름을 입력하세요"
}
```
...
| 상태        | 트리거 조건    | UI 표현                          |
| ----------- | -------------- | -------------------------------- |
| **Default** | 초기 렌더링    | 회색 배경 입력창 + Search 아이콘 |
...
- **Typography**: `Outfit`, Regular, `text-sm`, placeholder 색상 `text-gray-400`
- **Responsive**: 모든 해상도에서 `w-full` 유지
...
1. **Aroma (향)** 섹션: 9개 아로마 타입을 Chip 형태로 나열, 다중 선택 가능
2. **Flavor (맛)** 섹션: 쓴맛·단맛·산미를 각 1~5단계 Step Selector로 표시
3. **Body (바디감)** 섹션: 1(가벼움) ~ 3(묵직함) 3단계 Step Selector로 표시
4. **Roasting** 섹션: Light / Medium / Dark 3종 Chip, 다중 선택 가능
5. 하나 이상의 필터가 선택되면 상단에 "초기화" 버튼이 노출된다
6. 초기화 버튼 클릭 시 `onReset()`을 호출하여 모든 필터를 해제한다

#### 6. Design Spec (디자인 명세)

- **Layout**: `w-[240px] shrink-0`, 수직 스크롤 가능, 섹션 간 `pb-6 border-b`
- **Aroma Chip**: `rounded-full px-3 py-1 text-xs`, 선택 시 `bg-amber-500 text-white`, 미선택 시 `bg-gray-100 text-gray-600`
- **Step Selector**: 숫자 1~5 버튼, 선택 시 `bg-amber-500`, 미선택 시 `bg-gray-100`
- **Roasting Chip**: Aroma Chip과 동일 스타일
- **Typography**: 섹션 타이틀 `Outfit SemiBold text-xs uppercase tracking-widest text-gray-400`
- **Animation** (`framer-motion`): Chip 선택 시 `scale: 0.95 → 1.0` 0.1s 튕김 효과

#### 7. Definition of Done (검증 기준)

- [ ] (기능) Aroma Chip 다중 선택 및 해제가 정상 동작한다
- [ ] (기능) Flavor Step Selector가 1~5 단계 선택을 처리한다
- [ ] (기능) Body Step Selector가 1~3 단계 선택을 처리한다
...
2. Drawer 내부는 `BeanFilterPanel`과 동일한 필터 항목을 포함한다
...
- **Layout**: `fixed bottom-0 left-0 right-0`, 최대 높이 `80vh`, `rounded-t-2xl`, `bg-white`
...
beans/page.tsx (메인 엔트리)
  ├── BeanSearchBar         ← 검색창 (상단 헤더)
  ├── BeanFilterPanel       ← 필터 사이드바 (Desktop/Tablet only)
  ├── BeanFilterDrawer      ← 필터 Drawer (Mobile only)
  └── BeanCardList
        └── BeanCard × N
```
...
| 4   | **Flavor 필터 표현**   | Step Selector (버튼 1~5)        | Stepper 대신 Slider(Range Input) 사용 여부   |
