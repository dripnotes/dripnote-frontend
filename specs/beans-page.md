...
- **Layout**: `w-full md:max-w-xs`, `rounded-xl`, `h-12`, 좌측 아이콘 + 입력 텍스트 + 우측 Clear 버튼
...
- **Responsive**: 모바일에서는 `flex-1` 혹은 `w-full`, 데스크톱/태블릿에서는 `max-w-xs`
...
- [ ] (반응형) 데스크톱 환경에서 너비가 과하게 넓어지지 않게 제한된다
...
2. **Flavor (맛)** 섹션: 쓴맛·단맛·산미를 각 연속된 N등분 막대 형태(Rating Bar) 1~5단계로 표시
3. **Body (바디감)** 섹션: 1(가벼움) ~ 3(묵직함) 연속된 막대 형태(Rating Bar) 3단계로 표시
...
- **Rating Bar**: 회색 막대 배경(`bg-gray-100/30`), 값이 높을수록 진한 색상(`amber-200`~`amber-600` 등 분절되어 칠해짐), 미선택 마디는 `bg-gray-100`
