...
Baristation 서비스의 모든 페이지에서 일관된 사용자 경험(UX)과 시각적 정체성(Visual Identity)을 유지하기 위한 최상위 명세입니다. 개별 페이지 명세서(`specs/*.md`)는 본 공통 명세를 기반으로 작성되어야 합니다.
...
- 2026-04-20 (2차): 레이아웃 표준화를 위한 `SectionContainer` 도입 및 `RatingScale` 비활성 단계 시인성 개선 반영. (B안 리팩토링)
- 2026-04-20 (3차): 향미 지표 체계 개편(쓴맛 제거, 밸런스 추가) 및 모든 지표(산미, 단맛, 밸런스, 바디감, 로스팅)를 **5단계 척도**로 표준화함.
- 2026-04-21: (컴포넌트) `RatingScale` v4.6: `onChange`가 없을 경우 자동으로 `readOnly`로 동작하며 ARIA 속성(Group/GroupRole)을 강화한 접근성 표준 확립.
- 2026-05-05: 서비스명을 "Dripnote"에서 "Baristation"으로 전면 리브랜딩함. 이에 따라 모든 로고 텍스트 및 카피라이트 문구를 업데이트함. (리브랜딩 결정 반영)
