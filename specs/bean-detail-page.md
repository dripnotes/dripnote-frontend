...
1. 원두의 대표 아로마 이미지를 고해상도(1:1 비율)로 렌더링하며, 모든 해상도에서 수평 중앙 정렬을 유지한다.
2. 로스터리 명과 원두명을 명확한 위계(`Playfair Display`)로 표시한다.
3. 북마크 및 구매 아웃링크 버튼을 우측 정렬하여 시각적 리듬을 부여한다.
4. 상단에 "Back" 버튼을 두어 브라우저 History 상 이전 라우트로 이동 가능케 하며, 헤더와의 간격을 최소화(`mt-0 md:mt-4`)한다.
5. 모바일 해상도에서 이미지와 액션 버튼 사이의 간격을 좁게(`gap-4`) 조정하여 정보 밀도를 높인다.
